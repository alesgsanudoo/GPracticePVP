...
public class TimeUtil
{
    ...
    public static String readableTime(BigDecimal time) {
        ...
        if (time.doubleValue() <= 60.0) {
            ...
            return String.valueOf(text) + " " + time + "s";
        }
        if (time.doubleValue() <= 3600.0) {
            final int minutes = time.intValue() / 60;
            final int seconds = time.intValue() % 60;
            final DecimalFormat formatter = new DecimalFormat("00");
            return String.valueOf(text) + " " + formatter.format(minutes) + ":" + formatter.format(seconds) + "m";
        }
        return null;
    }
    ...
    public static String convertTime(long time) {
        ...
        String build = "";
        if (day > 0L) {
            build = String.valueOf(build) + day + "d, ";
        }
        if (hour > 0L) {
            build = String.valueOf(build) + hour + "h, ";
        }
        if (minute > 0L) {
            build = String.valueOf(build) + minute + "m, ";
        }
        if (second > 0L) {
            build = String.valueOf(build) + second + "s";
        }
        if (build.length() == 0) {
            build = "0s";
        }
