...
public final class ItemUtil
{
    ...
    public static String formatMaterial(final Material material) {
        ...
        name = name.replace('_', ' ');
        String result = new StringBuilder().append(name.charAt(0)).toString();
        for (int i = 1; i < name.length(); ++i) {
            if (name.charAt(i - 1) == ' ') {
                result = String.valueOf(result) + name.charAt(i);
            }
            else {
                result = String.valueOf(result) + Character.toLowerCase(name.charAt(i));
            }
        }
        return result;
    }
    ...
    public static ItemStack enchantItem(final ItemStack itemStack, final ItemEnchant... enchantments) {
        Arrays.<ItemEnchant>asList(enchantments).forEach(enchantment -> itemStack.addUnsafeEnchantment(enchantment.enchantment, enchantment.level));
        return itemStack;
    }
    ...
    public static ItemStack reloreItem(final ReloreType type, final ItemStack item, final String... lores) {
        ...
        switch (type) {
            case APPEND: {
                lore.addAll(Arrays.<String>asList(lores));
                meta.setLore((List)lore);
                break;
            }
            case PREPEND: {
                final List<String> nLore = new LinkedList<>(Arrays.<String>asList(lores));
                nLore.addAll(lore);
                meta.setLore((List)nLore);
                break;
            }
            case OVERWRITE: {
                meta.setLore((List)Arrays.<String>asList(lores));
                break;
            }
        }
        item.setItemMeta(meta);
    ...
    public enum ReloreType
    {
        ...
        private ReloreType(final String s, final int n) {
        }
    }
