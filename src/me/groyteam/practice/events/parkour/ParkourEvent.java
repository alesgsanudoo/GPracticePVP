...
public class ParkourEvent extends PracticeEvent<ParkourPlayer>
{
    ...
    public List<CustomLocation> getSpawnLocations() {
        return Collections.<CustomLocation>singletonList(this.getPlugin().getSpawnManager().getParkourLocation());
    }
    
    @Override
    public void onStart() {
        (this.gameTask = new ParkourGameTask()).runTaskTimerAsynchronously((Plugin)this.getPlugin(), 0L, 20L);
        (this.waterCheckTask = new WaterCheckTask()).runTaskTimer((Plugin)this.getPlugin(), 0L, 10L);
    ...
    private void giveItems(final Player player) {
        this.getPlugin().getServer().getScheduler().runTask((Plugin)this.getPlugin(), () -> {
            ...
            player.getInventory().setItem(0, ItemUtil.createItem(Material.FIREBALL, String.valueOf(ChatColor.GREEN.toString()) + ChatColor.BOLD + "Toggle Visibility"));
            player.getInventory().setItem(4, ItemUtil.createItem(Material.NETHER_STAR, String.valueOf(ChatColor.RED.toString()) + ChatColor.BOLD + "Leave Event"));
            player.updateInventory();
        });
    }
    
    private Player getRandomPlayer() {
        if (this.getByState(ParkourPlayer.ParkourState.INGAME).isEmpty()) {
            return null;
        }
